 ...
 * @package kufa
 */
if( !class_exists('kufa_Recent_Post') ){
	class kufa_Recent_Post extends WP_Widget{
		/**
		...
		function __construct(){
			...
			$widget_options = array(
				'description'					=> esc_html__('kufa recent post here', 'kufa'), 
				'customize_selective_refresh' 	=> true,
			);

			parent:: __construct('kufa_Recent_Post', esc_html__( 'Recent Post : kufa', 'kufa'), $widget_options );
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts','kufa' );
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'show_item' )); ?>"><?php echo esc_html__( 'No. of Item of posts to show:','kufa' ); ?></label>
...
function kufa_Recent_Post(){
	register_widget('kufa_Recent_Post');
}
add_action('widgets_init','kufa_Recent_Post');